...
export const ReportsPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"extinguishers" | "inspections" | "maintenance" | "expired">("extinguishers");
  ...
  const fetchReports = async () => {
    try {
      setIsLoading(true);
      const [extinguishers, inspections, maintenance, expired] = await Promise.all([
              ...
              <tbody>
                {extinguisherReports.map((extinguisher) => (
                  <tr key={extinguisher.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="py-3 px-4 text-sm text-text-primary">{extinguisher.serialNumber}</td>
                    ...
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        extinguisher.status === "ACTIVE" ? "bg-green-100 text-green-800" :
                        extinguisher.status === "EXPIRED" ? "bg-red-100 text-red-800" :
                        extinguisher.status === "MAINTENANCE_REQUIRED" ? "bg-yellow-100 text-yellow-800" :
                        "bg-gray-100 text-gray-800"
                      }`}>
                        {extinguisher.status}
                      </span>
                    </td>
                    <td className="py-3 px-4 text-sm text-text-primary">{extinguisher.expiryDate}</td>
                  </tr>
                ))}
              </tbody>
            </table>
                ...
                {inspectionReports.map((inspection) => (
                  <tr key={inspection.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="py-3 px-4 text-sm text-text-primary">
                      {inspection.extinguisher?.serialNumber} - {inspection.extinguisher?.location}
                    </td>
                    <td className="py-3 px-4 text-sm text-text-primary">{inspection.scheduledDate}</td>
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        inspection.status === "COMPLETED" ? "bg-green-100 text-green-800" :
                        inspection.status === "SCHEDULED" ? "bg-blue-100 text-blue-800" :
                        inspection.status === "CANCELLED" ? "bg-gray-100 text-gray-800" :
                        "bg-red-100 text-red-800"
                      }`}>
                        {inspection.status}
                      </span>
                    </td>
                    <td className="py-3 px-4 text-sm text-text-primary">{inspection.result || "-"}</td>
                    <td className="py-3 px-4 text-sm text-text-primary">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    <th className="text-left py-3 px-4 text-sm font-semibold text-text-primary">Type</th>
                    <th className="text-left py-3 px-4 text-sm font-semibold text-text-primary">Expiry Date</th>
                    <th className="text-left py-3 px-4 text-sm font-semibold text-text-primary">Days Expired</th>
                  </tr>
                </thead>
                <tbody>
                  {expiredExtinguishers.map((extinguisher) => {
                    ...
                    return (
                      <tr key={extinguisher.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4 text-sm text-text-primary">{extinguisher.location}</td>
                        <td className="py-3 px-4 text-sm text-text-primary">{extinguisher.type}</td>
                        <td className="py-3 px-4 text-sm text-red-600">{extinguisher.expiryDate}</td>
                        <td className="py-3 px-4">
