...
const getMyMaintenanceLogs = async (req, res) => {
    try {
        ...
        const limit = parseInt(req.query.limit) || 10;
        const skip = (page - 1) * limit;
        const maintenanceRecords = await shared_1.Maintenance.find({ inspectorId: req.user?.id })
            .populate('extinguisherId', 'serialNumber location type status')
            .skip(skip)
            .limit(limit)
            .sort({ dateOfAction: -1 });
        const total = await shared_1.Maintenance.countDocuments({ inspectorId: req.user?.id });
        res.json({
            maintenanceRecords,
            pagination: {
                page,
...
const getMaintenanceByExtinguisher = async (req, res) => {
    try {
        ...
        const limit = parseInt(req.query.limit) || 10;
        const skip = (page - 1) * limit;
        const maintenanceRecords = await shared_1.Maintenance.find({ extinguisherId: req.params.extinguisherId })
            .populate('inspectorId', 'firstName lastName email')
            .skip(skip)
            .limit(limit)
            .sort({ dateOfAction: -1 });
        const total = await shared_1.Maintenance.countDocuments({ extinguisherId: req.params.extinguisherId });
        res.json({
            maintenanceRecords,
            pagination: {
                page,
