...
export const deleteExtinguisher = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    try {
      await axios.delete(`${authUrl}/inspections/extinguisher/${req.params.id}`, {
        headers: { Authorization: req.header('Authorization') }
      });
    } catch (error) {
      logger.warn('Failed to delete related inspections', error);
    }

    // Delete maintenance via Maintenance Service
    try {
      await axios.delete(`${authUrl}/maintenance/extinguisher/${req.params.id}`, {
        headers: { Authorization: req.header('Authorization') }
      });
    } catch (error) {
      logger.warn('Failed to delete related maintenance records', error);
    }
